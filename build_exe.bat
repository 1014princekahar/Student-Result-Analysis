...
:: Copy icon for spec file resolution
copy /y assets\icon.ico build_out\assets\icon.ico >nul

:: Automatically close any running instances of the application to avoid WinError 5 (Access Denied)
echo.
...
pyinstaller --clean --noconfirm --onefile --windowed --add-data "%~dp0assets;assets" --add-data "%~dp0core;core" --icon=assets/icon.ico --name="StudentResultAnalyzer" --specpath="build_out" --workpath="build_out/build" --distpath="build_out/dist" main.py
goto :build_finish
